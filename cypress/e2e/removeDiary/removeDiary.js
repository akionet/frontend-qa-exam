...
When("I have {int} events created of {string} on {int} August", (count,eventType,dateOfFinalInterview) => {
  cy.setLocalStorage('count',count);
  cy.get('body').then((body)=>{
    if(body.find("div[class*='bg-red-200']").length===0){
      AddDiarySection.addEventsinDiary(dateOfFinalInterview,"2:00PM BST","3:00PM BST",eventType);
      AddDiarySection.addEventsinDiary(dateOfFinalInterview, "1:00PM BST","2:00PM BST",eventType);
      AddDiarySection.addEventsinDiary(dateOfFinalInterview,"3:30PM BST","4:30PM BST",eventType);
...
When("I remove event for time {string}", (starttime) => {
  RemoveEventMethods.selectEventandRemove(starttime,localStorage.getItem('count'));
})
...
When("I see event of {string} created",(eventType)=>{
  cy.get('body').then((body)=>{
    if(body.find("div[class*='bg-indigo-200']").length===0){
      AddDiarySection.addEventinDiary(10,SCREENING_EVENT_TITLE,"Screening Interview")
    }})

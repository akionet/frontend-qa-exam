class RemoveEventMethods{
    ...
    static chooseEventToBeDeleted(eventIdentifier){
        cy.xpath("//div[contains(text(),"+"'"+eventIdentifier+"'"+")]").click();
    ...
    static checkCountOfEventsPresentInRed(countOfEvents){
        cy.xpath("//div[contains(text(),'Final Interview Slot')]").and('have.length',countOfEvents);

    }
    static checkIfScreeningEventPresent(eventIdentifier){
        cy.xpath("//div[contains(text(),"+"'"+eventIdentifier+"'"+")]").should('be.visible')
    }

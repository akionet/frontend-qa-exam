...
class AddDiarySection{
   ...
   static selectDateinMiniCalendar(date){
    cy.xpath("//span[text()="+"'"+date+"']/parent::button").click({force:true});
   }
   static addEventinDiary(dateOfscreening,eventType,description){
    ...
    static assertDefaultColorForScreeningEvent(eventType,color){
        this.checkLabelsPresence(eventType).should('have.css','background-color', 'rgb(199, 210, 254)')
    ...
    static addEventsinDiary(dateOfFinalInterview,startTime,endTime,eventType){
    ...
    static assertFinalInterviewEventsCreated(eventType){
      this.checkLabelsPresence(eventType).should('exist').and('have.length.at.most',MAX_EVENTS);
        
    }
    static assertRedColorForFinalInterviewEvent(eventType,color){
        this.checkLabelsPresence(eventType).should('exist').and('have.css','background-color').and('eq','rgb(254, 202, 202)')
        this.checkColorLabels(color);
    }
